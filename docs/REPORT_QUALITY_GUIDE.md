...
모든 사실, 해석, 판정은 아래 신뢰 계약을 따라야 합니다.

| Label | 의미 |
|------|------|
| `verified` | 출처, 기준일, 근거가 추적 가능한 직접 사실 |
| `derived` | `verified` 사실에서 계산/정규화한 값 |
| `generated` | 규칙 또는 LLM이 근거 사실을 바탕으로 만든 해석 |
| `stale` | freshness window를 넘긴 근거에 의존하는 정보 |
| `unknown` | 근거 부족, 충돌, 누락 때문에 확정할 수 없는 정보 |

판정 라벨 `Go / Hold / Pass`는 **권고(advisory)** 전용입니다.
합격, 인터뷰, 장기 적합성을 보장하지 않으며 반드시 근거/불확실성과 함께 보여줘야 합니다.

공개 경계도 명확해야 합니다.

| Boundary | 규칙 |
|---------|------|
| `public_demo` | GitHub Pages 등 공개 데모에 노출 가능한 검증된 공개 스냅샷만 허용 |
| `internal_only` | 운영자 검토/개발/QA 전용. 공개 배포 금지 |
| `private_user` | 이력서, JD, 지원 메모 등 사용자 유래 데이터. 공개 경로 배포 절대 금지 |
