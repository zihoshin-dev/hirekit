...
class TestNormalizeSections:
    ...
    def test_section_1_employees_aggregated(self):
        raw = {
            1: {
                "employees": [
                    {"headcount": "1,000", "avg_tenure_year": "3.5",
                     "total_salary": "5000000000", "position": "개발", "gender": "남"},
                    {"headcount": "500", "avg_tenure_year": "2.8",
                     "total_salary": "2000000000", "position": "개발", "gender": "여"},
                ]
            }
        }
        result = normalize_sections(raw)
    ...
    def test_section_1_news_truncated_to_5_items(self):
        raw = {
            1: {
                "recent_news": [
                    {"title": f"뉴스 {i}", "source": "test", "link": "http://x.com"}
                    for i in range(10)
                ]
            }
        }
    ...
    def test_section_1_news_html_cleaned(self):
        raw = {
            1: {
                "recent_news": [
                    {"title": "<b>카카오</b> 뉴스", "source": "연합", "link": "http://x.com"}
                ]
            }
        }
        result = normalize_sections(raw)
    ...
    def test_section_4_culture_normalization(self):
        raw = {
            4: {
                "naver_blog": [
                    {"title": "<b>카카오</b> 후기", "description": "좋은 회사  입니다"}
                    for _ in range(7)
                ]
            }
        }
