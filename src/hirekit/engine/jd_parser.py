...
_SECTION_PATTERNS: list[tuple[str, list[str]]] = [
    ("required", [
        r"자격\s*요건", r"필수\s*요건", r"지원\s*자격", r"필수\s*자격",
        r"requirements?", r"qualifications?", r"must\s*have",
        r"필수\s*조건", r"기본\s*자격",
    ]),
    ("preferred", [
        r"우대\s*사항", r"우대\s*조건", r"우대\s*자격",
        r"preferred", r"nice[\s\-]to[\s\-]have", r"bonus", r"플러스",
        r"우대", r"가점",
    ]),
    ("responsibilities", [
        r"담당\s*업무", r"주요\s*업무", r"업무\s*내용", r"수행\s*업무",
        r"이런\s*일", r"하는\s*일", r"역할",
        r"responsibilities?", r"what\s*you.ll\s*do", r"your\s*role",
        r"job\s*duties", r"duties",
    ]),
    ("tech_stack", [
        r"기술\s*스택", r"사용\s*기술", r"기술\s*환경", r"개발\s*환경",
        r"tech\s*stack", r"technologies?", r"tools?\s*&\s*technologies?",
    ]),
    ("team", [
        r"팀\s*소개", r"팀\s*정보", r"조직\s*소개", r"우리\s*팀",
        r"about\s*the\s*team", r"our\s*team",
    ]),
]
...
class JDParser:
    ...
    def _parse_sections(self, jd: ParsedJD, lines: list[str]) -> None:
        ...
        jd._tech_stack_lines = section_buffers["tech_stack"]  # type: ignore[attr-defined]
    ...
    def _extract_tech_stack(self, jd: ParsedJD) -> None:
        ...
        tech_stack_lines = getattr(jd, "_tech_stack_lines", [])
        for tech in _extract_from_lines(tech_stack_lines):
            if tech not in preferred_set:
                required_techs.append(tech)

        # Deduplicate required, then exclude anything already in preferred
        jd.required_tech = [
            t for t in dict.fromkeys(required_techs) if t not in preferred_set
        ]
