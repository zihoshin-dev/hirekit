...
_GROUNDING_RULES = (
    "중요: 수집된 데이터에 없는 사실은 만들어내지 마세요. 각 주장에는 반드시 출처를 [소스명] 형태로 명시하세요."
)
...
class LLMPipeline:
    ...
    @staticmethod
    def _format_source_chunk(result: SourceResult) -> str:
        label = _SECTION_LABELS.get(result.section, result.section)
        freshness = "stale" if result.is_stale else "current"
        source_authority = result.source_authority or "secondary_research"
        return (
            f"[출처: {result.source_name} / {label} / trust: {result.trust_label} / "
            f"authority: {source_authority} / freshness: {freshness}]\n{result.raw[:800]}"
        )
    ...
    def _extract_facts(self, raw_results: list[SourceResult], company: str) -> str:
        ...
        for r in raw_results:
            if not r.raw:
                continue
            raw_chunks.append(self._format_source_chunk(r))
